...
function buttonClickEvent(event) {
  ...
  // plus-minus
  if (className.includes("plus-minus")) return plus_minusBtnClick();

  // percentage
  if (className.includes("percentage")) return percentageBtnClick();
...
function operatorBtnClick(value) {
  // 2+2+2
...
function plus_minusBtnClick() {
  const currentNumber = Number(result.innerText);
  result.innerText = String(currentNumber * -1);
}

function percentageBtnClick(){
  const currentNumber = Number(result.innerText);
  result.innerText = String(currentNumber * 0.01)
}